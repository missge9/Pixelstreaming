...
function stopInterval() {
    clearInterval(startInterval); // stoppt das Interval
}
  
let IsRendering = false;
let canResize = new Boolean(false);
let startDelay = false;



function SendToUE(category) {
    const descriptor = {
            ButtonInteraction: category,
    };
    this.stream.emitUIInteraction(descriptor);
    console.log("sucsess sending to unreal" + category);
}

function isMobileOrTablet() {
    // Überprüfe, ob es sich um einen mobilen oder Tablet-Browser handelt
    return /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
}


const Touchelement = document.getElementById('appRealityStream');
// Event-Handler für Touch-Ereignisse
Touchelement.addEventListener('touchstart', function(e) {
  console.log("Touch-Ereignis erkannt.");
  SendToUE("mobile")
});

// Event-Handler für Mausereignisse
Touchelement.addEventListener('mousedown', function(e) {
  console.log("Mausereignis erkannt.");
  SendToUE("Notmobile")
});
...
let afk = {
    ...
    warnTimeout: 600,   // The time to elapse before warning the user they are inactive.
...
function populateDefaultProtocol() {
    ...
    fromStreamerMessages.add("QualityControlOwnership", 1);
...
function showTextOverlay(text,isShowError) {
    let textOverlay = document.createElement('div');
    textOverlay.id = 'messageOverlay';
    if(isShowError && startDelay){
        textOverlay.innerHTML = null ;
        var  divError = document.createElement('h2');
        divError.innerText = "Verbindungsfehler!";
        textOverlay.appendChild(divError);
        var p1 = document.createElement("div");
        p1.innerText = "Die Internetverbindung zu RealityStream wurde unterbrochen. ";
        textOverlay.appendChild(p1);
        var  p2 = document.createElement("div");
        p2.innerText = "Sobald die Verbindung wieder hergestellt ist, wird Ihre RealityStream Session fortgesetzt. ";
        textOverlay.appendChild(p2);

        var p3 = document.createElement("div");
        p3.innerText = "Dies dauert im Normalfall nur wenige Sekunden. Bitte warten Sie daher einen Moment ";
        textOverlay.appendChild(p3);

        var p4 = document.createElement("div");
        p4.innerText = "und schließen Sie das Fenster nicht.";
        textOverlay.appendChild(p4);

        textOverlay.appendChild(document.createElement("br"));
        var p5 = document.createElement("div");
        p5.innerText = "Wir danken Ihnen für Ihre Geduld und Ihr Verständnis.";
        textOverlay.appendChild(p5);
...
function updateAfkOverlayText() {
    afk.overlay.innerHTML = '<center>Keine Aktivität entdeckt. Disconnected in: ' + afk.countdown + ' seconds<br>Klicke um fortzufahren<br></center>';
}
...
function resizePlayerStyleToFillWindow(playerElement) {
    let videoElement = playerElement.getElementsByTagName("player");
...
function resizePlayerStyleToActualSize(playerElement) {
    ...
    if (videoElement.length > 0) {
        ...
        var  playerElementWidth = styleWidth -300;
        if(styleWidth<=0)
        //Video is now 100% of the playerElement, so set the playerElement style
         playerElement.style = "top: " + styleTop + "px; left: " + styleLeft + "px; width: " + playerElementWidth + "px; height: " + styleHeight + "px; cursor: " + styleCursor + "; " + styleAdditional;
        else 
          playerElement.style = "width：100%;height:100%;cursor:"+styleCursor+styleAdditional;
    }
}
...
function resizePlayerStyleToArbitrarySize(playerElement) {
    ...
    //Video is now 100% of the playerElement, so set the playerElement style
    playerElement.style = "top: 0px; left: 0px; width: " + playerElementWidth + "px; height: " + styleHeight + "px; cursor: " + styleCursor + "; " + styleAdditional;
}
...
function connect() {
    ...
    var url =$("#publicip").val();//"https://realitystream.dvdarch.ch"
    
    let connectionUrl = url.replace('http://', 'ws://').replace('https://', 'wss://');
    ...
    ws.onmessage = function(event) {
        ...
        inputOptions.suppressBrowserKeys = true;
    ...
    ws.onclose = function(event) {
        ...
        closeStream();
        // var errText = "<h2>Verbindungsfehler!</h2><p>Die Internetverbindung zu RealityStream wurde unterbrochen. <p>";
        if(ws.attemptStreamReconnection === true){
            console.log(`WS closed: ${JSON.stringify(event.code)} - ${event.reason}`);
            if(event.reason !== "")
            {
                showTextOverlay(`DISCONNECTED: ${event.reason.toUpperCase()}`,true);
            }
            else
            {
                showTextOverlay(`DISCONNECTED`,true);
            }
...
function delayStart(){
    if(__ButtonState == ButtonStateEnum.ELoadding){
        ...
        // toastr.success ("RealityStream ist bereit! <br />Klicken Sie hier um zu starte");
...
window.onresize = function(){
    if(canResize == true){
        console.log("Fenster hat jetzt eine andere grösse!!");
    }
}
...
function load() {
    ...
    addResponseEventListener('logListener', (response) => {
        ...
        if (response === "IsRendering") {
            console.log("Debug-Nachricht: Der Streamer rendert gerade.");
            IsRendering = true;
        }
        if (response === "FinishedRendering") {
            console.log("Debug-Nachricht: Rendering ist fertig.");
            IsRendering = false;
            updateVideoStreamSize();
        }
    });   
